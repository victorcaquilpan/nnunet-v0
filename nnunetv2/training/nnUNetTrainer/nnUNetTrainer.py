...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
        ...
        self.initial_lr = 1e-2 # originall 1e-2 
        self.weight_decay = 3e-5 # originally 3e-5
        self.oversample_foreground_percent = 0.33 # originally 0.33
        ...
        if self.label_manager.has_regions:
            loss = DC_and_BCE_loss({},
                                   {'batch_dice': self.configuration_manager.batch_dice,
                                    'do_bg': True, 'smooth': 1e-6, 'ddp': self.is_ddp},
                                   use_ignore_label=self.label_manager.ignore_label is not None,
                                   dice_class=MemoryEfficientSoftDiceLoss)
        else:
            loss = DC_and_CE_loss({'batch_dice': self.configuration_manager.batch_dice,
                                   'smooth': 1e-6, 'do_bg': False, 'ddp': self.is_ddp}, {}, weight_ce=1, weight_dice=1,
    ...
    def do_split(self):
        ...
        if self.fold == "all":
            ...
            if not isfile(splits_file):
                ...
                splits = generate_crossval_split(all_keys_sorted, seed=11, n_splits=3)
            ...
            if self.fold < len(splits):
                ...
                rnd = np.random.RandomState(seed=11 + self.fold)
    ...
    def on_validation_epoch_end(self, val_outputs: List[dict]):
        ...
        eps = 1e-8
        global_dc_per_class = [i for i in [2 * i / (2 * i + j + k + eps) for i, j, k in zip(tp, fp, fn)]]
